...
```{r}
heat_murders <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/pg_murder_temps.csv")
```
...
```{r}
heat_murders |> summarize(coorelation = cor(murders, temperature, method = "pearson"))
```
...
The correlation and scatterplot suggest that the warmer months are positively correlated with a higher murder rate. This makes the claim that the summer will bring an increased murder rate legitimate and actually based in reality.
