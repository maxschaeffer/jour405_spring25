...
Certain groups of education levels might be more or less inclined to participate in surveys than others. My guess is that those with more education may typically be more likely to want to participate in a survey than vice versa.
...
Voters with more education more likely to vote in upcoming election, according to survey
...
High school or less showed the biggest change with almost 3% less in the "yes" category. Both of the other two categories stayed pretty close to the original number

2. Why might the weighted results be considered more accurate than the unweighted results?

The weighted results are considered more accurate, because without applying the weights you can be dealing with a sample that is not representative of the population you are trying to generate data on.
...
No, because something seems to be wrong with the code where instead of filtering the weighted responses to a percentage of yes's, it set all the weighted values to zero. I'm not sure how to fix this. 
...
This data shows us that more educated people are more likely to vote in elections or at least in this specific election that the poll was taken in regards to. The poll shows that all three education levels have a majority that plan to vote vs. not. 

The poll also showed that those without college education were less likely to vote after the results were weighted for particiapation. This means that thos ewithout college educations were less likely overall to participate in the survey. It is important for news reporters to have an understanding of if the results they are looking at are weighted or not, and if they are weighted what groups are underrepresnted and what is the potential context/reasoning for that. 
...
It is important because if journalists doesn't know whether they're looking at weighted or unweighted results to a poll, they could lack an understanding of a certain groups actual results. For example in this data if you looked at the unweighted numbers for people who haven't been to college you would think that a higher percentage of them are planning to vote than when you account for the number of them that answered the survey.
2. How might the differences between weighted and unweighted results affect how you would report on this data?

I think the differences would allow you to provide the audience context about which groups were easeier to get responses from for the survey and which were harder.
...
I'd want to know the raw data of survey respondents from each category, and the formula for how they weighted them.
