...
Some variables that seem to be correlated are percent White and percent of employment or unemployment, percent white and turnout, percent of employment and turnout, and income and percent bachelors or higher.

The only one that seems to have absolutely zero coorelation are percent bachelors and median age. Most variables seem to have at least a somewhat linear relationship with turnout with a fex exceptions, the least linear seems to be population.
...
This model is predicting voter turnout very well according to the output. I would consider it useful, because both R-Squared values are above 80% meaning a large amount of turnout is explained by the model and it does a good job accounting for the variety of variables. Our F-Statistic is also demonstrating that the model is statistically significant as a whole. The p-values are also demonstrating that percent bachelors, population, and percent unemployed are not as statisticaly significant.
...
I tried removing the three variables that came back statistically insignificant, but none worked better than our original model surprisingly..
...
Statistical analysis demonstrated that certain factors had a much larger effect than others on voter turnout in populations across the United States. Income, age, and racial demographicsh had a significant effect on the number of voters participating in elections. 
